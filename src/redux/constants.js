export const SIGN_UP_FOR_SAGA = 'SIGN_UP_FOR_SAGA'
export const DO_LOG_IN_FOR_SAGA = 'DO_LOG_IN_FOR_SAGA'
export const ADD_NEW_LIST_FOR_SAGA = 'ADD_NEW_LIST_FOR_SAGA'
export const ADD_NEW_ITEM_FOR_SAGA = 'ADD_NEW_ITEM_FOR_SAGA'
export const CHANGE_ITEM_FOR_SAGA = 'CHANGE_ITEM_FOR_SAGA'
export const DELETE_ITEM_FOR_SAGA = 'DELETE_ITEM_FOR_SAGA'
export const DELETE_LIST_FOR_SAGA = 'DELETE_LIST_FOR_SAGA'

export const ADD_USER_TO_STORE = 'ADD_USER_TO_STORE'
export const ADD_LISTS_TO_STORE = 'ADD_LISTS_TO_STORE'
export const ADD_PAGETITLE_TO_STORE = 'ADD_PAGETITLE_TO_STORE'
// export const ADD_ITEM_TO_STORE = 'ADD_ITEM_TO_STORE'
// export const ADD_ONE_LIST_TO_STORE = 'ADD_ONE_LIST_TO_STORE'

export const MESSAGE_ON = 'MESSAGE_ON'
export const MESSAGE_OFF = 'MESSAGE_OFF'